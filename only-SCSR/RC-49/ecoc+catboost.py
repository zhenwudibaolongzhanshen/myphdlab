# ============================================================
# ecoc_catboost_after_pca.py
# 功能：在 PCA 降维后的特征上，使用 ECOC 软分类概率筛选样本，
#       然后训练 CatBoost 回归模型，评估并保存结果。
...
# 输出目录：catboost_filtered_pca_[scheme]/
# ============================================================
...
def visualize_results(results, feature_names, output_dir, scheme_desc):
    ...
    plt.suptitle(f'CatBoost 回归结果 - {scheme_desc} (PCA后)')
    plt.tight_layout()
    ...
    plt.xlabel('特征重要性')
    plt.title(f'CatBoost特征重要性 - {scheme_desc} (PCA后)')
    plt.tight_layout()
...
def main():
    print("=" * 80)
    print("ECOC筛选 + CatBoost回归 (PCA后) - 糖尿病数据集")
    print("=" * 80)
    ...
    # 输出目录基础
    output_base_dir = 'catboost_filtered_pca'
