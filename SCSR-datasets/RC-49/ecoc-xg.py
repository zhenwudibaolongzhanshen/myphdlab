# ============================================================
# xgboost_diabetes_pca_with_ecoc_filter.py
# 功能：在 PCA 降维后的特征上，使用 ECOC 软分类概率筛选样本，
#       然后训练 XGBoost 回归模型（max_depth=4, num_boost_round=1000），
#       评估并保存结果。
# 依赖：需先运行 ecoc_mlp_after_pca.py，生成模型文件：
#       ecoc_pca_output/ecoc_mlp_pca.pkl
# 输入：train_features.npy, train_labels.npy,
#       val_features.npy,   val_labels.npy,
#       test_features.npy,  test_labels.npy
# 输出目录：xgboost_filtered_pca_[scheme]/
# ============================================================
...
def train_and_evaluate(X_train, y_train, X_val, y_val, X_test, y_test,
    ...
    use_gpu = False
    try:
        dtrain_tiny = xgb.DMatrix(X_train[:10], label=y_train[:10])
        params_tiny = {
            'objective': 'reg:squarederror',
            'max_depth': 4,
            'device': 'cuda',          # 新版本推荐使用 device 参数
            'tree_method': 'hist',
            'verbosity': 0
        }
    ...
    # ---------- 设置XGBoost参数（深度4，迭代1000，早停50） ----------
    params = {
        'objective': 'reg:squarederror',
        'learning_rate': 0.01,
        'max_depth': 4,                 # 固定深度4
        'lambda': 50,                    # 正则项，与原XGBoost脚本一致
        'min_child_weight': 10,
        'subsample': 1.0,
    ...
    # ---------- 训练模型（带早停）----------
    # 注意：num_boost_round设为1000，早停50轮
    model = xgb.train(
        params=params,
        dtrain=dtrain,
...
def save_results(results, feature_names, output_dir, scheme_desc):
    ...
    # 4. 保存特征重要性（使用 weight 类型，也可改为 gain）
...
def visualize_results(results, feature_names, output_dir, scheme_desc):
    ...
    plt.suptitle(f'XGBoost 回归结果 - {scheme_desc} (PCA后)')
    plt.tight_layout()
    ...
    plt.xlabel('特征重要性')
    plt.title(f'XGBoost特征重要性 - {scheme_desc} (PCA后)')
    plt.tight_layout()
...
def main():
    print("=" * 80)
    print("ECOC筛选 + XGBoost回归 (PCA后) - 糖尿病数据集")
    print("=" * 80)

    # 加载ECOC模型
    ecoc_model_path = r'ecoc_pca_output/ecoc_mlp_pca.pkl'
    if not os.path.exists(ecoc_model_path):
        raise FileNotFoundError(f"请先训练ECOC模型: {ecoc_model_path}")
    ...
    # 输出目录基础
    output_base_dir = 'xgboost_filtered_pca'
