...
# -------------------- 数据加载（测试集）--------------------
print("加载 PCA 降维后的测试数据...")
X_test = np.load('test_features.npy')
y_test = np.load('test_labels.npy')

# 加载训练时保存的标准化器
scaler = torch.load('scaler_pca.pth', weights_only=False)  # 注意 weights_only=False
...
# 加载模型
input_dim = np.load('input_dim_pca.npy')[0].item()
model = SelectiveNetRegressor(input_dim, hidden_dims=[128, 64, 32])
model.load_state_dict(torch.load('selectivenet_pca_model.pth', map_location='cpu'))
model.eval()
print("模型加载完成")

# 加载验证集置信度
confs_val = np.load('val_confs_pca.npy')
print(f"验证集置信度统计: min={confs_val.min():.4f}, max={confs_val.max():.4f}, "
      f"mean={confs_val.mean():.4f}, median={np.median(confs_val):.4f}")
...
# -------------------- 多覆盖率遍历 --------------------
coverages_to_try = np.linspace(0.1, 0.9, 9)
results = []
...
# -------------------- 绘制曲线 --------------------
metrics_info = {
    'RMSE': {'title': 'Coverage-RMSE Curve (PCA Data)', 'ylabel': 'RMSE (Lower is Better)'},
    'MAE': {'title': 'Coverage-MAE Curve (PCA Data)', 'ylabel': 'MAE (Lower is Better)'},
    'R2': {'title': 'Coverage-R² Curve (PCA Data)', 'ylabel': 'R² (Higher is Better)'},
    'MAPE': {'title': 'Coverage-MAPE Curve (PCA Data)', 'ylabel': 'MAPE (%) (Lower is Better)'}
}
