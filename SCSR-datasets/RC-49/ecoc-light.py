# ============================================================
# lightgbm_diabetes_pca_with_ecoc_filter.py
# 功能：在 PCA 降维后的特征上，使用 ECOC 软分类概率筛选样本，
#       然后训练 LightGBM 回归模型，评估并保存结果。
...
# 输出目录：lightgbm_filtered_pca_[scheme]/
# ============================================================
...
def train_and_evaluate(X_train, y_train, X_val, y_val, X_test, y_test,
    ...
    try:
        # 尝试用一个小数据集测试GPU是否可用
        train_data_tiny = lgb.Dataset(X_train[:10], label=y_train[:10])
        params_tiny = {
    ...
    # ---------- 设置LightGBM参数（参照原LightGBM代码，适当调整） ----------
    params = {
        'objective': 'regression',
        'metric': 'rmse',
        'boosting_type': 'gbdt',
...
def save_results(results, feature_names, output_dir, scheme_desc):
    ...
    with open(params_path, 'w', encoding='utf-8') as f:
        f.write(f"LightGBM模型参数 - {scheme_desc} (PCA后)\n")
        f.write("=" * 50 + "\n")
...
def visualize_results(results, feature_names, output_dir, scheme_desc):
    ...
    plt.suptitle(f'LightGBM 回归结果 - {scheme_desc} (PCA后)')
    plt.tight_layout()
    ...
    plt.title(f'LightGBM特征重要性 - {scheme_desc} (PCA后)')
    plt.tight_layout()
...
def main():
    print("=" * 80)
    print("ECOC筛选 + LightGBM回归 (PCA后) - 糖尿病数据集")
    print("=" * 80)
    ...
    # 输出目录基础
    output_base_dir = 'lightgbm_filtered_pca'
    ...
    for scheme_id, filter_func, scheme_desc in SCHEMES:
        ...
        # 训练并评估（传入 numpy 数组，与catboost版本一致）
        train_and_evaluate(
