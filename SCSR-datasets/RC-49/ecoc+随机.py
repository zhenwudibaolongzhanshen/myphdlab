# ============================================================
# random_forest_diabetes_pca_with_ecoc_filter.py
# 功能：在 PCA 降维后的特征上，使用 ECOC 软分类概率筛选样本，
#       然后训练随机森林回归模型（n_estimators=1000, max_depth=4），
#       评估并保存结果。
...
# 输出目录：random_forest_filtered_pca_[scheme]/
# ============================================================
...
def train_and_evaluate(X_train, y_train, X_val, y_val, X_test, y_test,
    ...
    # 创建随机森林模型（参数：树1000，深度4，其余与原脚本一致）
    model = RandomForestRegressor(
        n_estimators=1000,
        max_depth=4,                     # 固定深度4，与CatBoost对齐
        min_samples_leaf=50,
...
def save_results(results, feature_names, output_dir, scheme_desc):
    ...
    with open(params_path, 'w', encoding='utf-8') as f:
        f.write(f"随机森林模型参数 - {scheme_desc} (PCA后)\n")
        f.write("=" * 50 + "\n")
        ...
        f.write(f"树的数量: 1000\n")
        f.write(f"最大深度: 4\n")
        f.write(f"最小叶子样本数: 50\n")
        f.write(f"内部节点再划分所需最小样本数: 10\n")
        f.write(f"\n模型评估结果:\n")
...
def visualize_results(results, feature_names, output_dir, scheme_desc):
    ...
    plt.suptitle(f'随机森林回归结果 - {scheme_desc} (PCA后)')
    plt.tight_layout()
    ...
    plt.xlabel('特征重要性')
    plt.title(f'随机森林特征重要性 - {scheme_desc} (PCA后)')
    plt.tight_layout()
...
def main():
    print("=" * 80)
    print("ECOC筛选 + 随机森林回归 (PCA后) - 糖尿病数据集")
    print("=" * 80)
    ...
    # 输出目录基础
    output_base_dir = 'random_forest_filtered_pca'
