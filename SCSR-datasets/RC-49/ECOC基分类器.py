# ============================================================
# ecoc_logistic_filtered_catboost.py
# 功能：加载以逻辑回归为基分类器的ECOC模型，利用软分类概率筛选样本，
...
# 输出目录：catboost_filtered_pca_logistic_<方案>/
# ============================================================
...
class ECOCBaseTrainer:
    def __init__(self, code_matrix, boundaries, n_segments, base_model_type='logistic',
                 feature_names=None, random_state=42):
        self.code_matrix = code_matrix
        self.boundaries = boundaries
        self.n_segments = n_segments
        self.base_model_type = base_model_type.lower()
        self.feature_names = feature_names if feature_names is not None else [f'PC{i+1}' for i in range(code_matrix.shape[1])]
        self.code_length = code_matrix.shape[1]
        self.random_state = random_state
        self.scaler = None  # 加载后会被填充
    ...
    def predict_proba(self, X):
        ...
        for col, clf in enumerate(self.binary_classifiers):
            if clf is None:
                ...
                # 对于逻辑回归/随机森林，predict_proba 返回 [P(0), P(1)]，取第二列
...
def condition4_top1_gt(prob, threshold=0.75):
...
def train_and_evaluate(X_train, y_train, X_val, y_val, X_test, y_test,
    ...
    print(f"\n{'=' * 50}")
    print(f"基分类器: logistic | 方案：{scheme_desc}")
    print(f"{'=' * 50}")
    ...
    save_results(results, feature_names, output_dir, scheme_desc, 'logistic')
    return results
...
def main():
    print("=" * 80)
    print("逻辑回归基分类器ECOC筛选 + CatBoost回归 (PCA后) - 糖尿病数据集")
    print("=" * 80)

    # 加载ECOC模型（逻辑回归基分类器）
    ecoc_model_path = r'ecoc_randomforest_output/ecoc_randomforest_pca.pkl'
    ...
    print(f"ECOC模型加载成功，分段数: {n_segments}，基分类器类型: {trainer.base_model_type}")
    ...
    output_base_dir = 'catboost_filtered_pca_catboost'
