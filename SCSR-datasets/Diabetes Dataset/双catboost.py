# ============================================================
# catboost_diabetes_with_ecoc_filter_catboost.py
# 功能：在糖尿病数据集上，使用基于CatBoost的ECOC软分类概率筛选样本，
#       然后训练CatBoost回归模型，评估并保存结果。
# 依赖：需先运行 ECOC+catboost分类.py 训练脚本，生成模型文件：
#       ecoc_diabetes_output/ecoc_catboost_diabetes.pkl
# ============================================================
...
# ==================== 重新定义 ECOCCatBoostTrainer 类（与第一阶段一致，仅保留预测方法）====================
    ...
    def predict_proba(self, X):
        ...
        for col, clf in enumerate(self.binary_classifiers):
            if clf is None:
                ...
                # CatBoost的predict_proba返回(n_samples, 2)，取第二列（正类）
...
def main():
    print("=" * 80)
    print("ECOC筛选（CatBoost基分类器） + CatBoost回归 - 糖尿病数据集")
    print("=" * 80)

    # 加载ECOC模型（CatBoost版）
    ecoc_model_path = r'ecoc_diabetes_output/ecoc_catboost_diabetes.pkl'
    if not os.path.exists(ecoc_model_path):
        raise FileNotFoundError(f"请先训练ECOC-CatBoost模型: {ecoc_model_path}")
    ...
    # 划分数据集（与ECOC训练时一致：70%训练，20%验证，10%测试，随机种子42）
    ...
    # 输出目录基础（添加_catboost后缀以区别于其他基分类器版本）
    output_base_dir = 'catboost_filtered_diabetes_catboost'
