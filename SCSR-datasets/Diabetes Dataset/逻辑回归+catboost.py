# ============================================================
# catboost_diabetes_with_ecoc_filter_logistic.py
# 功能：在糖尿病数据集上，使用基于逻辑回归的ECOC软分类概率筛选样本，
#       然后训练CatBoost回归模型，评估并保存结果。
# 依赖：需先运行 ECOC+逻辑回归.py 训练脚本，生成模型文件：
#       ecoc_diabetes_output/ecoc_logistic_diabetes.pkl
# ============================================================
...
# ==================== 重新定义 ECOCLogisticTrainer 类（与第一阶段一致，仅保留预测方法）====================
...
def condition4_top1_gt(prob, threshold=0.45):
    """条件4：最高概率大于阈值"""
...
def main():
    print("=" * 80)
    print("ECOC筛选（逻辑回归基分类器） + CatBoost回归 - 糖尿病数据集")
    print("=" * 80)

    # 加载ECOC模型（逻辑回归版）
    ecoc_model_path = r'ecoc_diabetes_output/ecoc_logistic_diabetes.pkl'
    if not os.path.exists(ecoc_model_path):
        raise FileNotFoundError(f"请先训练ECOC逻辑回归模型: {ecoc_model_path}")
    ...
    # 输出目录基础（添加_logistic后缀以区别于MLP版本）
    output_base_dir = 'catboost_filtered_diabetes_logistic'
