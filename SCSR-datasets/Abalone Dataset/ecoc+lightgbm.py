# ============================================================
# lightgbm_abalone_with_ecoc_filter.py
# 功能：在鲍鱼数据集（含Sex独热编码）上，使用ECOC软分类概率筛选样本，
#       然后训练LightGBM回归模型（GPU加速），评估并保存结果。
...
def train_and_evaluate(X_train, y_train, X_val, y_val, X_test, y_test,
    ...
    try:
        # 尝试用一个小数据集测试GPU是否可用
        train_data_tiny = lgb.Dataset(X_train.iloc[:10], label=y_train.iloc[:10])
        params_tiny = {
        ...
        lgb.train(params_tiny, train_data_tiny, num_boost_round=1)
        use_gpu = True
    ...
    params = {
        ...
        'num_leaves': 31,
        'max_depth': 10,
        'min_child_samples': 10,       # 对应 min_data_in_leaf
...
def save_results(results, feature_names, output_dir, scheme_desc):
    ...
    imp_path = os.path.join(output_dir, 'lightgbm_feature_importance.csv')
    ...
    with open(params_path, 'w', encoding='utf-8') as f:
        f.write(f"LightGBM模型参数 - {scheme_desc}\n")
        f.write("=" * 50 + "\n")
...
def visualize_results(results, feature_names, output_dir, scheme_desc):
    ...
    plt.suptitle(f'LightGBM 回归结果 - {scheme_desc}')
    plt.tight_layout()
    ...
    plt.title(f'LightGBM特征重要性 - {scheme_desc}')
    plt.tight_layout()
...
def main():
    print("=" * 80)
    print("ECOC筛选 + LightGBM回归 - 鲍鱼数据集 (保留Sex列并独热编码)")
    print("=" * 80)
    ...
    # 转换为DataFrame并确保类型为 float（LightGBM 要求）
    ...
    # 输出目录基础
    output_base_dir = 'lightgbm_filtered_abalone_with_sex'
...
if __name__ == "__main__":
    print(f"当前LightGBM版本: {lgb.__version__}")
    main()