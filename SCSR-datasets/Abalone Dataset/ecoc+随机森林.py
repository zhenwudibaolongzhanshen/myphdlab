# ============================================================
# random_forest_abalone_with_ecoc_filter.py
# 功能：在鲍鱼数据集（含Sex独热编码）上，使用ECOC软分类概率筛选样本，
#       然后训练随机森林回归模型，评估并保存结果。
...
def save_results(results, feature_names, output_dir, scheme_desc):
    ...
    imp_path = os.path.join(output_dir, 'random_forest_feature_importance.csv')
    ...
    with open(params_path, 'w', encoding='utf-8') as f:
        f.write(f"随机森林模型参数 - {scheme_desc}\n")
        f.write("=" * 50 + "\n")
...
def visualize_results(results, feature_names, output_dir, scheme_desc):
    ...
    plt.suptitle(f'随机森林回归结果 - {scheme_desc}')
    plt.tight_layout()
    ...
    plt.xlabel('特征重要性')
    plt.title(f'随机森林特征重要性 - {scheme_desc}')
    plt.tight_layout()
...
def main():
    print("=" * 80)
    print("ECOC筛选 + 随机森林回归 - 鲍鱼数据集 (保留Sex列并独热编码)")
    print("=" * 80)
    ...
    # 转换为DataFrame并确保类型为 float（随机森林可接受）
    ...
    # 输出目录基础
    output_base_dir = 'random_forest_filtered_abalone_with_sex'
