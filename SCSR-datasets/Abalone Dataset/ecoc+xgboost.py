# ============================================================
# xgboost_abalone_with_ecoc_filter.py
# 功能：在鲍鱼数据集（含Sex独热编码）上，使用ECOC软分类概率筛选样本，
#       然后训练XGBoost回归模型，评估并保存结果。
...
def condition4_top1_gt(prob, threshold=0.5):
...
def train_and_evaluate(X_train, y_train, X_val, y_val, X_test, y_test,
    ...
    use_gpu = False
    try:
        dtrain_tiny = xgb.DMatrix(X_train.iloc[:10], label=y_train.iloc[:10])
        params_tiny = {
        ...
        xgb.train(params_tiny, dtrain_tiny, num_boost_round=1)
        use_gpu = True
        print("   ✅ GPU可用，将使用GPU加速")
    except Exception as e:
        print(f"   ⚠ GPU不可用，使用CPU训练（错误信息: {e}）")
    ...
    params = {
        ...
        'learning_rate': 0.01,
        'max_depth': 10,
        'lambda': 50,                # reg_lambda
        'min_child_weight': 10,       # min_child_weight
        'subsample': 1.0,
...
def save_results(results, feature_names, output_dir, scheme_desc):
    ...
    importance = np.array([importance_dict.get(f, 0) for f in feature_names])
    imp_df = pd.DataFrame({
        'feature': feature_names,
        'importance': importance
    }).sort_values('importance', ascending=False)
    imp_path = os.path.join(output_dir, 'xgboost_feature_importance.csv')
    ...
    with open(params_path, 'w', encoding='utf-8') as f:
        f.write(f"XGBoost模型参数 - {scheme_desc}\n")
        f.write("=" * 50 + "\n")
        f.write(f"使用的特征数量: {len(feature_names)}\n")
        f.write(f"随机种子: 42\n")
        f.write(f"数据分割比例: 训练集70%, 验证集20%, 测试集10%\n")
        f.write(f"最佳迭代轮数: {results.get('best_iteration', 'N/A')}\n")
        f.write(f"\n模型评估结果:\n")
...
def visualize_results(results, feature_names, output_dir, scheme_desc):
    ...
    plt.suptitle(f'XGBoost 回归结果 - {scheme_desc}')
    plt.tight_layout()
    ...
    # 特征重要性图
    importance_dict = results['model'].get_score(importance_type='weight')
    importance = np.array([importance_dict.get(f, 0) for f in feature_names])
    ...
    plt.xlabel('特征重要性')
    plt.title(f'XGBoost特征重要性 - {scheme_desc}')
    plt.tight_layout()
...
def main():
    print("=" * 80)
    print("ECOC筛选 + XGBoost回归 - 鲍鱼数据集 (保留Sex列并独热编码)")
    print("=" * 80)
    ...
    # ========== 关键修改：将所有特征和目标转换为 float 类型，避免 XGBoost 数据类型错误 ==========
    X_train = pd.DataFrame(X_train, columns=feature_names).astype(float)
    X_val = pd.DataFrame(X_val, columns=feature_names).astype(float)
    X_test = pd.DataFrame(X_test, columns=feature_names).astype(float)
    y_train = pd.Series(y_train, name='target').astype(float)
    y_val = pd.Series(y_val, name='target').astype(float)
    y_test = pd.Series(y_test, name='target').astype(float)
    ...
    # 输出目录基础
    output_base_dir = 'xgboost_filtered_abalone_with_sex'
...
if __name__ == "__main__":
    print(f"当前XGBoost版本: {xgb.__version__}")
    main()