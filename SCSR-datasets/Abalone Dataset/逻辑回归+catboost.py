# ============================================================
# catboost_abalone_with_ecoc_filter_logistic.py
# 功能：在鲍鱼数据集上，使用第一阶段ECOC-Logistic模型概率筛选样本，
#       然后训练CatBoost回归模型。
# 依赖：需先运行第一阶段脚本 ecoc+logistic.py，生成模型文件：
#       ecoc_abalone_with_sex_output_logistic/ecoc_logistic_abalone.pkl
# ============================================================
...
# ==================== 定义与第一阶段完全一致的 ECOCLogisticTrainer 类 ====================
...
def main():
    print("=" * 80)
    print("ECOC-Logistic筛选 + CatBoost回归 - 鲍鱼数据集")
    print("=" * 80)

    # 加载第一阶段模型（逻辑回归版本）
    ecoc_model_path = r'ecoc_abalone_with_sex_output_logistic/ecoc_logistic_abalone.pkl'
    if not os.path.exists(ecoc_model_path):
        raise FileNotFoundError(f"请先训练ECOC-Logistic模型: {ecoc_model_path}")
    ...
    output_base_dir = 'catboost_filtered_abalone_logistic'
