# ============================================================
# catboost_abalone_with_ecoc_filter_rf.py
# 功能：在鲍鱼数据集上，使用第一阶段ECOC-RF模型概率筛选样本，
#       然后训练CatBoost回归模型。
# 依赖：需先运行第一阶段脚本 ecoc+rf.py，生成模型文件：
#       ecoc_abalone_with_sex_output_rf/ecoc_rf_abalone.pkl
# ============================================================
    ...
    def predict_proba(self, X):
        ...
        for col, clf in enumerate(self.binary_classifiers):
            if clf is None:
                ...
                # 随机森林的 predict_proba 返回形状 (n_samples, 2)
...
def main():
    print("=" * 80)
    print("ECOC-RF筛选 + CatBoost回归 - 鲍鱼数据集")
    print("=" * 80)

    # 加载第一阶段模型（随机森林版本）
    ecoc_model_path = r'ecoc_abalone_with_sex_output_rf/ecoc_rf_abalone.pkl'
    if not os.path.exists(ecoc_model_path):
        raise FileNotFoundError(f"请先训练ECOC-RF模型: {ecoc_model_path}")
    ...
    # 输出目录基础
    output_base_dir = 'catboost_filtered_abalone_rf'
