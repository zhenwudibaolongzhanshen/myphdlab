"""SOGN Split-Sample on ApplianceEnergy — LSTM backbone.

训练集 50/50 分割：一半训门控（序数头），一半训回归。
验证 double-dipping 批评是否在 SOGN 最强数据集上成立。
"""
import numpy as np
import torch
...
from shared.data_utils import quantile_bins, to_bin_labels, build_lstm_encoder
from data_utils import load_appliance_energy_bike_style

CONFIG = {
    'method': 'sogn_split_sample',
    'lstm_hidden': 256, 'lstm_layers': 2, 'lstm_dropout': 0.2,
...
def train_one_seed(seed):
    ...
    split = n_train // 2
    gate_idx = indices[:split]
    reg_idx = indices[split:]

    X_gate, y_gate, b_gate = X_train[gate_idx], y_train[gate_idx], train_bin[gate_idx]
    X_reg, y_reg, b_reg = X_train[reg_idx], y_train[reg_idx], train_bin[reg_idx]
    ...
    for epoch in range(CONFIG['pretrain_epochs']):
        ...
        if (epoch + 1) % 10 == 0:
            val_bin = torch.tensor(to_bin_labels(y_val.numpy().squeeze(), bin_edges),
    ...
    # ---- 阶段三：冻结 gate + ord_head，微调 LSTM + reg_net（仅 reg half）----
    ...
    print(f'  R² (full test): {r2_score(y_true, y_pred):.4f}')
