"""Train SOGN on Bike Sharing with 64-dim x 1-layer LSTM — v2: improved tuning.

Key changes from v1:
  1. Longer pretrain (60 epochs) — give ordinal classifier more time with small backbone
  2. Lower lambda_cov (0.1) — reduce coverage pressure, let regression breathe
  3. More joint epochs (150) — compensate for slower convergence
  4. Joint finetune — don't freeze gate, use small ordinal loss to keep it grounded
  5. Lower finetune LR (1e-4)
"""
import numpy as np
import torch
...
CONFIG = {
    ...
    'lambda_cov': 0.1, 'target_coverage': 0.50,
    'pretrain_epochs': 60, 'epochs': 150, 'finetune_epochs': 60,
    'batch_size': 64, 'lr': 1e-3, 'ft_lr': 1e-4, 'ord_loss_weight': 0.1,
    'seeds': [42, 43, 44],
}

RESULT_DIR = os.path.join(os.path.dirname(__file__), 'results', 'sogn_64x1_v2')
...
def train_one_seed(seed):
    ...
    # ---- Phase 1: Extended ordinal pretrain (60 epochs) ----
    ...
    # ---- Phase 2: Extended joint training (150 epochs) ----
    ...
    # ---- Phase 3: Joint finetune (don't freeze gate, low LR + small ordinal loss) ----
    model.load_state_dict(best_state)
    opt_ft = torch.optim.Adam(model.parameters(), lr=CONFIG['ft_lr'])
    ...
    for epoch in range(CONFIG['finetune_epochs']):
        ...
        for xb, yb, bb in train_loader:
            xb, yb, bb = xb.to(DEVICE), yb.to(DEVICE), bb.to(DEVICE)
            y_hat, w, p = model(xb)
            # Weighted regression + light ordinal loss to prevent classifier drift
            loss_reg = (w.detach() * (y_hat - yb).pow(2).squeeze()).mean()
            loss_ord = CONFIG['ord_loss_weight'] * F.cross_entropy(p, bb)
            opt_ft.zero_grad(); (loss_reg + loss_ord).backward(); opt_ft.step()
        model.eval()
        with torch.no_grad():
