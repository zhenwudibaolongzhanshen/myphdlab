"""Train SOGN on Bike Sharing with 64-dim x 1-layer LSTM — v3.

Key insight: gate quality is limited by 64-dim backbone (ord acc ~65%).
v3 strategy: freeze LSTM+gate during regression finetune, train reg_net
with plain MSE (no w weighting). This recovers full-coverage R² while
preserving gate rankings for selective prediction.
"""
import numpy as np
import torch
...
CONFIG = {
    ...
    'pretrain_epochs': 60, 'epochs': 150, 'finetune_epochs': 80,
    'batch_size': 64, 'lr': 1e-3, 'ft_lr': 3e-4,
    'seeds': [42, 43, 44],
}

RESULT_DIR = os.path.join(os.path.dirname(__file__), 'results', 'sogn_64x1_v3')
...
def train_one_seed(seed):
    ...
    # ---- Phase 3: RegNet-only finetune (freeze LSTM + gate + ord_head) ----
    # Key insight: gate quality is limited by 64-dim backbone, but we know
    # gate RANKING is good (low-coverage R2 proves it). So freeze everything
    # except reg_net, train with plain MSE. This recovers full-coverage R2
    # without disturbing the selective prediction signal.
    ...
    opt_ft = torch.optim.Adam(model.reg_net.parameters(), lr=CONFIG['ft_lr'])
    ...
    for epoch in range(CONFIG['finetune_epochs']):
        ...
        for xb, yb, bb in train_loader:
            ...
            y_hat, _, _ = model(xb)
            loss = F.mse_loss(y_hat, yb)  # plain MSE, no gate weighting
